...
namespace ExploracionPlanes
{
    public static class Chequeos
    {
        ...
        public static string tomografiaVieja(PlanSetup plan)
        {
            ...
            {
                DateTime fechaTomo = (DateTime)plan.StructureSet.Image.CreationDateTime;
        ...
        public static string planNoCalculado(PlanSetup plan)
        {
            string texto = "";
            if (plan.Dose == null)
            {
                texto += "\n" + plan.Id + ": el plan no está calculado";
            }
            return texto;
        }
        ...
        public static string chequeos(PlanningItem plan, bool esPlanSuma)
        {
            ...
            if (!esPlanSuma)
            {
                ...
                texto += planNoCalculado((PlanSetup)plan);
            }
            else
            {
                ...
                foreach (PlanSetup etapa in ((PlanSum)plan).PlanSetups)
                {
                    ...
                    aux += orientacionArcos(etapa);
                    texto += planNoCalculado(etapa);
                    if (aux != "")
                    {
                        texto += "\n" + etapa.Id + ": " + aux + "\n";
                    }
